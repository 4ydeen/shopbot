...
دو پلیس‌هولدر دیگر هم همیشه در دسترس‌اند (برای درگاه‌هایی که به‌جای/علاوه‌بر
credentials صاحب‌حساب، یک شناسه‌ی مشتری هم در بدنه/هدر/URL می‌خواهند):
- {customer_user_id}: آیدی عددی تلگرام کاربر؛ خودکار از پروفایل کاربر گرفته
  می‌شود، نیازی به پرسیدن از کاربر نیست.
- {customer_phone}: شماره موبایل کاربر؛ فقط وقتی «require_customer_phone» در
  تنظیمات همین درگاه فعال باشد از کاربر (با دکمه‌ی اشتراک‌گذاری شماره‌ی
  تلگرام) گرفته و پر می‌شود، وگرنه رشته‌ی خالی است.
"""
